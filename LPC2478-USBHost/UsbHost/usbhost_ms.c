...
USB_INT32S  MS_ParseConfiguration (void)
{
    ...
    ///refer to the USB2.0 spec. P264 	 9.6.3 Configuration
    desc_ptr     = TDBuffer;
    ms_int_found = 0;
    ...
    desc_ptr += desc_ptr[0];/*跳转到下个配置(configuration, interface,endpoint, and class- or vendor-specific)*/
...
//  while (desc_ptr != TDBuffer + *((USB_INT16U *) &TDBuffer[2])) {
        ...
        switch (desc_ptr[1]) {
            ...
            case USB_DESCRIPTOR_TYPE_INTERFACE:                       /* If it is an interface descriptor   */
                 ...
                 /*貌似如果跳转到此处就无法跳出这个循环  下面的else为我自己增加*/
                 else
                 {
                	 desc_ptr += desc_ptr[0];  /* Move to next descriptor start      */
                 }
