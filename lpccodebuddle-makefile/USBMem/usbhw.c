...
#pragma arm section zidata = "USB_RAM"      // data located in USB RAM (neccessry for DMA)
                                            // see file DMA.sct
DWORD UDCA[USB_EP_NUM];                     // USB Device communication Area first in USB RAM
DWORD DD_NISO_Mem[4*DD_NISO_CNT];           // Non-Iso DMA Descriptor Memory (4 DWORDs each)
DWORD DD_ISO_Mem [5*DD_ISO_CNT];            //     Iso DMA Descriptor Memory (5 DWORDs each)
...
void USB_Reset (void) {
...
#else
  EP_INT_EN   = 0xFFFFFFFF;                            // ignore USB_DMA_EP if  USB_DMA not configured
#endif
...
#if USB_DMA
  UDCA_HEAD   = USB_RAM_ADR;                            // USB Device Communication Area start address
  ...
  DMA_INT_EN  = (SYS_ERR_INT | NDD_REQ_INT | EOT_INT);  // Enable DMA interrupts
  DDMemMap[0] = 0x00000000;
  DDMemMap[1] = 0x00000000;
  for (n = 0; n < USB_EP_NUM; n++) {
    udca[n] = 0;
    UDCA[n] = 0;                                        // clear USB Device Communication Area
  }
#endif
}
...
#if USB_DMA
  if (USB_DMA_IsUsed (EPNum)) {                         // Check if EP is used for DMA
    cnt = USB_DMA_BufCnt (EPNum);                       // get length of received data
                                                        // data is already copied to BulkBuf
    return (cnt);
  }
#endif
...
DWORD USB_WriteEP (DWORD EPNum, BYTE *pData, DWORD cnt) {
  ...
  if (USB_DMA_IsUsed (EPNum)) {                         // Check if EP is used for DMA
    DD.BufAdr  = (DWORD)pData;                          // DMA Buffer Address
    DD.BufLen  = cnt;                                   // DMA Buffer Length in bytes
    DD.MaxSize = 64 /* MSC_MAX_PACKET */;               // Maximum packet size
    DD.Cfg.Val = 0;                                     // Initial DMA Configuration
    DD.Cfg.Type.IsoEP = 0;                              // Non-Iso Endpoint

    USB_DMA_Setup (EPNum, &DD);                         // Setup DMA
    USB_DMA_Enable(EPNum);                              // Enable DMA
    USB_DMA_ReqSet(EPNum);                              // set DMA request
    return (cnt);
  } 
#endif
...
const DWORD DDSz [2] = { (4 * 4),     (5 * 4)    };     // Non-Iso 4 DWORDs, Iso 5 DWORDs
...
BOOL USB_DMA_Setup(DWORD EPNum, USB_DMA_DESCRIPTOR *pDD) {
  ...
  *(((DWORD *)nxt)++) =  pDD->Cfg.Type.LenPos << 8; /* If ATLE selected */
  if (iso) {
...
/*
 *  DMA interrupt request set for  USB DMA Endpoint
 *    Parameters:      EPNum: Endpoint Number
 *                       EPNum.0..3: Address
 *                       EPNum.7:    Dir
 *    Return Value:    None
...
void USB_DMA_ReqSet (DWORD EPNum) {
  DMA_REQ_SET = 1 << EPAdr(EPNum);
}


/*
 *  check if DMA transfer is used for USB Endpoint
 *    Parameters:      EPNum: Endpoint Number
 *                       EPNum.0..3: Address
 *                       EPNum.7:    Dir
 *    Return Value:    TRUE - used for DMA, FALSE - not used for DMA
 */

BOOL USB_DMA_IsUsed (DWORD EPNum) {
  DWORD EPPhyAdr;

  EPPhyAdr = EPAdr(EPNum);                              // Endpoint's Physical Address
  if ((1 << EPPhyAdr) & USB_DMA_EP)                     // EP bit set in DMA EP table ?
    return (TRUE);
  else
    return (FALSE);
}
