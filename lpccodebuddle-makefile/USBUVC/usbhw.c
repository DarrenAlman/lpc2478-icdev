...
#pragma diag_suppress 1441
  ...
  typedef struct _EP_TRANSFER {
    U16 cnt;
    U8  dir;     /* write = 1, read = 2, 3 = isr */
    U8  EpNum;   /* endpoint number     */
    U16 len;
    U8  dat[USB_MAX_PACKET0];   /* data */
  } EP_TRANSFER;

  #define EP_TRANS_MAX 350
  EP_TRANSFER EpTrans[EP_TRANS_MAX];
  U16         EpTransCnt = 0;
  U8         *pEpDat;
#endif //__TEST__
...
#ifdef  __TEST__
  if (/*(num == 0) &&*/ EpTransCnt < EP_TRANS_MAX) {
    EpTrans[EpTransCnt].cnt = EpTransCnt;
    EpTrans[EpTransCnt].dir = 1;
    EpTrans[EpTransCnt].EpNum = EPNum;
    EpTrans[EpTransCnt].len = cnt;
    for (n = 0; n < cnt; n++) {
      EpTrans[EpTransCnt].dat[n] = *pEpDat++;
    }
    EpTransCnt++;
  }
#endif //__TEST__
