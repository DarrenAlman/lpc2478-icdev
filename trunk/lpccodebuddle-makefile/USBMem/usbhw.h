...
#define DD_NISO_SZ    (DD_NISO_CNT * ( 4*4))            // Non-Iso DMA Descr. Size (4 DWORDs each) in bytes
#define DD_ISO_SZ     (DD_ISO_CNT  * ( 5*4))            //     Iso DMA Descr. Size (5 DWORDs each) in bytes
#define DD_NISO_ADR   (USB_RAM_ADR + (32*4))            // Non-Iso DMA Descr. Address (starts after UDCA (32 DWORDs))
#define DD_ISO_ADR    (DD_NISO_ADR + DD_NISO_SZ)        //     Iso DMA Descr. Address */
#define DD_SZ         ((32*4) + DD_NISO_SZ + DD_ISO_SZ) // complete DMA Descr. Size incl. UDCA
...
extern void  USB_DMA_ReqSet (DWORD EPNum);
extern BOOL  USB_DMA_IsUsed (DWORD EPNum);
