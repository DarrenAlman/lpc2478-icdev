...
typedef struct fixed_memoryblock_management_block {
	uint_t		next;			/* 次の未割当てブロック *//*下一个未分配的块*/
} MPFMB;
...
typedef struct fixed_memorypool_initialization_block {
	...
	uint_t		blksz;			/* メモリブロックのサイズ（丸めた値） */ /*内存块的大小（四舍五入值）*/
	void		*mpf;			/* 固定長メモリプール領域の先頭番地 */ /*固定大小的内存池区的起始地址*/
	MPFMB		*p_mpfmb;		/* 固定長メモリプール管理領域の先頭番地 */ /*固定长度的内存池管理区的起始地址*/
} MPFINIB;
...
typedef struct fixed_memorypool_control_block {
	...
	uint_t		fblkcnt;		/* 未割当てブロック数 */   /*未分配块*/
	uint_t		unused;			/* 未使用ブロックの先頭 */ /*未使用的块的顶部*/
	uint_t		freelist;		/* 未割当てブロックのリスト */ /*一个未分配的块的列表*/
} MPFCB;
...
typedef struct fixed_memorypool_waiting_information {
	WINFO		winfo;			/* 標準の待ち情報ブロック */ /*标准信息块等待*/
	MPFCB		*p_mpfcb;		/* 待っている固定長メモリプールの管理ブロック*/
	void		*blk;			/* 獲得したメモリブロック */ /*获得的内存块*/
} WINFO_MPF;
