 ...
 *  の責任を負わない．
 * 
 *  @(#) $Id: task.c 1483 2009-03-17 06:42:56Z ertl-hiro $
 */
 ...
 *  タスク例外処理ルーチンの呼出し
 *
 *  ASPカーネルでは，タスク例外処理ルーチン内でCPUロック状態に移行し，
 *  元の状態に戻さずにリターンした場合，カーネルが元の状態に戻す．
 *
 *  「p_runtsk->enatex = false」をループの中に置いているのは，タスク例
 *  外処理ルーチンから，タスク例外処理許可状態のままリターンした場合の
 *  対策である．逆に，これを元に戻すための「p_runtsk->enatex = true」を
 *  ループの外に置いているのは，ループするとすぐに「p_runtsk->enatex =
 *  false」を実行するためである．dispatchは，「p_runtsk->enatex = true」
 *  の前に呼び出すべきである．
 */
